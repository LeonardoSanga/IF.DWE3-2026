...
function dw3IsLogged() {
    ...
    const isLogged = document.cookie.split(';').some((cookie) => cookie.trim().startsWith('IsLogged='));
...
function dw3ClearIsLoggedCookie() {
    ...
    var isLogged = document.cookie.split(';').some((cookie) => cookie.trim().startsWith('IsLogged='));
